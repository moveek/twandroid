...
def find_package(setup, found_package):
    if found_package:
        ...
        print "dirs in this dir: %s" % get_dirs()
        count = 0
        package = []
        for d in get_dirs():
            os.chdir(d)
            count = count + 1
            print "count: %s, cwd: %s" % (count, os.getcwd())
            #print "exclude: %s" % get_exclude_dirs(setup)
...
def copy_package(setup):
    ...
    try:
        ...
        # modules into it.
        copy_into_from(top_level_package, install_target)
